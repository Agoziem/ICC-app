const SortContacts = (contacts) => {
    return contacts.sort((a, b) => {
        ...
        ? new Date(a.recieved_messages[a.recieved_messages.length - 1].timestamp)
        : new Date(0);
    
        const aLastSentMessage = a.sent_messages.length
        ? new Date(a.sent_messages[a.sent_messages.length - 1].timestamp)
        : new Date(0);
        ...
        ? new Date(b.recieved_messages[b.recieved_messages.length - 1].timestamp)
        : new Date(0);
    
        const bLastSentMessage = b.sent_messages.length
        ? new Date(b.sent_messages[b.sent_messages.length - 1].timestamp)
        : new Date(0);
